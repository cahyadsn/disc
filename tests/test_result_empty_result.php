...
class MockStmt {
    ...
    public function execute() {
        ...
        return true; // Simulate execute success
    }

    public function get_result() {
        $this->get_result_count++;
        // Return valid result object, but it will yield no rows
...
if ($db->stmt->get_result_count === 1) {
    echo "PASS: get_result() called once.\n";
} else {
    echo "FAIL: get_result() was called " . $db->stmt->get_result_count . " times.\n";
    $failed = true;
}

if (strpos($output, 'Data not found, check your database.') !== false) {
    echo "PASS: HTML output contains error message when query returns empty result.\n";
} else {
